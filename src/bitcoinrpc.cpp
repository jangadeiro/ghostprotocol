...
void PrintConsole(const char* format, ...)
{
    ...
    MyMessageBox(buffer, "Ghostprotocol", wxOK | wxICON_EXCLAMATION);
#else
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop ghostprotocol server.");

    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "ghostprotocol server stopping";
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Ghostprotocol address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Ghostprotocol address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <ghostprotocoladdress> <account>\n"
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount <ghostprotocoladdress>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <ghostprotocoladdress> <amount> [comment] [comment-to]\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <ghostprotocoladdress> [minconf=1]\n"
            "Returns the total amount received by <ghostprotocoladdress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toghostprotocoladdress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.01"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        CScript scriptPubKey;
        if (!scriptPubKey.SetBitcoinAddress(strAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Ghostprotocol address:")+strAddress);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Ghostprotocol server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <ghostprotocoladdress>\n"
            "Return information about <ghostprotocoladdress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ghostprotocol is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ghostprotocol is downloading blocks...");
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <ghostprotocolprivkey> [label] [rescan=true]\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importaddress <ghostprotocoladdress> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <ghostprotocoladdress>\n"
            "Reveals the private key corresponding to <ghostprotocoladdress>."
            + HelpRequiringPassphrase());
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <ghostprotocoladdress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <ghostprotocoladdress> <signature> <message>\n"
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!IsValidBitcoinAddress(address))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Ghostprotocol address: ")+input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!IsValidBitcoinAddress(address))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Ghostprotocol address: ")+s.name_);
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: ghostprotocol-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg)
{
    if (nStatus == 401)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: ghostprotocol-json-rpc/%s\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    {
        string strWhatAmI = "To use ghostprotocold";
...
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but ghostprotocol compiled without full openssl libraries.");
#endif
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    if (strPrint != "")
    {
        ...
        MyMessageBox(strPrint, "Ghostprotocol", wxOK);
#else
