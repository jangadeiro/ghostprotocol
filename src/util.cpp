...
#else
    const char* pszModule = "ghostprotocol";
#endif
    if (pex)
...
#ifdef GUI
    if (wxTheApp && !fDaemon)
        MyMessageBox(pszMessage, "Ghostprotocol", wxOK | wxICON_ERROR);
#endif
    throw;
}
...
string GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\Ghostprotocol
    // Mac: ~/Library/Application Support/Ghostprotocol
    // Unix: ~/.ghostprotocol
#ifdef __WXMSW__
    // Windows
...
{
    string confFile = GetConfigFile("ghostprotocol.conf");
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "ghostprotocold.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Ghostprotocol will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(MyMessageBox, strMessage+" ", string("Ghostprotocol"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }
