...
const int GHOST_PROTOCOL_GENESIS_EXTRA = 521;
uint256 hashGhostProtocolGenesisBlock("0xbf67dbeacfe0c17815788e71619dab48fa711962a1dd4507dda4af583f91399f");

class CGhostprotocolHooks : public CHooks
{
public:
...
string SendMoneyWithInputTx(const CScript& scriptPubKey, int64 nValue, int64 nNetFee, const CWalletTx& wtxIn, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired))
        return "ABORTED";
#else
    if (fAskFee && !ThreadSafeAskFee(nFeeRequired, "Ghostprotocol", NULL))
        return "ABORTED";
#endif
...
Value sendtoname(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoname <ghostprotocolname> <amount> [comment] [comment-to]\n"
    ...
    if (!AddressToHash160(strAddress, hash160))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No valid ghostprotocol address");
...
Value name_firstupdate(const Array& params, bool fHelp)
{
    ...
    CWalletTx wtx;
    wtx.nVersion = GHOSTPROTOCOL_TX_VERSION;
...
Value name_update(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 3)
    {
        ...
        bool isValid = AddressToHash160(strAddress, hash160);
        if (!isValid)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ghostprotocol address");
        scriptPubKeyOrig.SetBitcoinAddress(strAddress);
    }
    else
    {
  ...
  if (!IsValidBitcoinAddress (address))
    {
      ...
      msg << "Invalid Ghostprotocol address: " << address;
      throw JSONRPCError (RPC_INVALID_ADDRESS_OR_KEY, msg.str ());
    }

  tx.nVersion = GHOSTPROTOCOL_TX_VERSION;
...
bool CNameDB::ReconstructNameIndex()
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
    {
        while (pindex)
        {
            ...
            {
                if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
                    continue;
...
CHooks* InitHook()
{
    ...
    hashGenesisBlock = hashGhostProtocolGenesisBlock;
    printf("Setup ghostprotocol genesis block %s\n", hashGenesisBlock.GetHex().c_str());
    return new CGhostprotocolHooks();
}

bool CGhostprotocolHooks::IsStandard(const CScript& scriptPubKey)
{
    return true;
}
...
void CGhostprotocolHooks::AddToWallet(CWalletTx& wtx)
{
}

bool CGhostprotocolHooks::IsMine(const CTransaction& tx)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return false;
...
bool CGhostprotocolHooks::IsMine(const CTransaction& tx, const CTxOut& txout, bool ignore_name_new /* = false*/)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return false;
...
bool
CGhostprotocolHooks::AcceptToMemoryPool (DatabaseSet& dbset, const CTransaction& tx)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return true;
...
void CGhostprotocolHooks::RemoveFromMemoryPool(const CTransaction& tx)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return;
...
bool GetNameOfTx(const CTransaction& tx, vector<unsigned char>& name)
{
    ...
    if (!good)
        return error("GetNameOfTx() : could not decode a ghostprotocol tx");
...
IsConflictedTx (DatabaseSet& dbset, const CTransaction& tx, vchType& name)
{
    ...
    if (!good)
        return error("IsConflictedTx() : could not decode a ghostprotocol tx");
    int nPrevHeight;
    int nDepth;
    int64 nNetFee;
...
bool
CGhostprotocolHooks::ConnectInputs (DatabaseSet& dbset,
                               map<uint256, CTxIndex>& mapTestPool,
                               const CTransaction& tx,
                               vector<CTransaction>& vTxPrev,
                               vector<CTxIndex>& vTxindex,
                               ...
                               bool fBlock, bool fMiner)
{
    ...
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
    {
        ...
        if (foundOuts && (!fBlock || doStrictChecks (pindexBlock->nHeight)))
            return error("ConnectInputHook: non-Ghostprotocol tx has name outputs");

        // Make sure name-op outputs are not spent by a regular transaction, or the name
        // would be lost
        if (found)
            return error("ConnectInputHook() : a non-ghostprotocol transaction with a ghostprotocol input");
        return true;
    }
    ...
    if (!good)
        return error("ConnectInputsHook() : could not decode a ghostprotocol tx");
    ...
    switch (op)
    {
        case OP_NAME_NEW:
            if (found)
                return error("ConnectInputsHook() : name_new tx pointing to previous ghostprotocol tx");
...
bool
CGhostprotocolHooks::DisconnectInputs (DatabaseSet& dbset, const CTransaction& tx,
                                  CBlockIndex* pindexBlock)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return true;
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs, pindexBlock->nHeight);
    if (!good)
        return error("DisconnectInputsHook() : could not decode ghostprotocol tx");
    if (op == OP_NAME_FIRSTUPDATE || op == OP_NAME_UPDATE)
    {
        //vector<CDiskTxPos> vtxPos;
        vector<CNameIndex> vtxPos;
...
bool CGhostprotocolHooks::CheckTransaction(const CTransaction& tx)
{
    if (tx.nVersion != GHOSTPROTOCOL_TX_VERSION)
        return true;
...
bool CGhostprotocolHooks::ExtractAddress(const CScript& script, string& address)
{
...
bool
CGhostprotocolHooks::ConnectBlock (CBlock& block, DatabaseSet& dbset,
                              CBlockIndex* pindex)
{
    return true;
}
...
bool
CGhostprotocolHooks::DisconnectBlock (CBlock& block, DatabaseSet& dbset,
                                 CBlockIndex* pindex)
{
    return true;
}
...
bool GenesisBlock(CBlock& block, int extra)
{
    ...
    txNew.vout[0].scriptPubKey = CScript() << ParseHex("1230369050cd899ffbbc4e8ee51e8c4534a855bb463439d63d235d4779685d8b6f4870a238cf365ac94fa13ef9a2a22cd99d0d5ee86dcabcafce36c7acf43ce5") << OP_CHECKSIG;
...
bool CGhostprotocolHooks::GenesisBlock(CBlock& block)
{
    if (fTestNet)
        return false;

    return ::GenesisBlock(block, GHOST_PROTOCOL_GENESIS_EXTRA);
}

int CGhostprotocolHooks::LockinHeight()
{
    if (fTestNet)
        return 0;
...
bool CGhostprotocolHooks::Lockin(int nHeight, uint256 hash)
{
    if (!fTestNet)
        if (nHeight == 2016 && hash != uint256("0x"))
            return false;
    return true;
}

string CGhostprotocolHooks::IrcPrefix()
{
...
unsigned char GetAddressVersion() { return ((unsigned char)(fTestNet ? 38 : 38)); }
