...
bool GetMyExternalIP(unsigned int& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CAddress("104.18.59.232", 80); // www.showmyip.com
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
	string strDesc = "Ghostprotocol " + FormatFullVersion();
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Ghostprotocol is probably already running."), ntohs(sockaddr.sin_port));
        else
