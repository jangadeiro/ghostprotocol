...
#include "ghostprotocol.h"    // For DecodeNameScript in GetAmounts to correctly compute credit/debit
...
{
  if (nVersion != GHOSTPROTOCOL_TX_VERSION)
    return false;
...
string CWallet::SendMoneyToBitcoinAddress(string strAddress, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!scriptPubKey.SetBitcoinAddress(strAddress))
        return _("Invalid ghostprotocol address");
