...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("ghostprotocol version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  ghostprotocol [options]                   \t  " + "\n" +
            "  ghostprotocol [options] <command> [params]\t  " + _("Send command to -server or ghostprotocold") + "\n" +
            "  ghostprotocol [options] help              \t\t  " + _("List commands") + "\n" +
            "  ghostprotocol [options] help <command>    \t\t  " + _("Get help for a command") + "\n";

        strUsage += "\n" + HelpMessage();

#if defined(__WXMSW__) && defined(GUI)
        // Tabs make the columns line up in the message box
        wxMessageBox(strUsage, "Ghostprotocol", wxOK);
#else
        // Remove tabs
    ...
    if (!fDebug && !pszSetDataDir[0])
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("ghostprotocol version %s\n", FormatFullVersion().c_str());
//#ifdef GUI
#if 0
...
//#if defined(__WXMSW__) && defined(GUI)
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindowA("wxWindowClassNR", "Ghostprotocol");
            if (hwndPrev)
            {
    ...
    if (!lock.try_lock())
    {
        wxMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  ghostprotocol is probably already running."), GetDataDir().c_str()), "Ghostprotocol");
        return false;
    }
    ...
    if (!fNoListen)
    {
        ...
        {
            wxMessageBox(strErrors, "Ghostprotocol");
            return false;
        }
    }
    ...
    //
    if (fDaemon)
        fprintf(stdout, "ghostprotocol server starting\n");
    strErrors = "";
    int64 nStart;
    ...
    if (mapArgs.count("-mininput"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -mininput=<amount>"), "Ghostprotocol");
            return false;
        }
    }
    ...
    if (!strErrors.empty())
    {
        wxMessageBox(strErrors, "Ghostprotocol", wxOK | wxICON_ERROR);
        return false;
    }
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            wxMessageBox(_("Invalid -proxy address"), "Ghostprotocol");
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -paytxfee=<amount>"), "Ghostprotocol");
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            wxMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), "Ghostprotocol", wxOK | wxICON_EXCLAMATION);
    }
    ...
    if (!CreateThread(StartNode, NULL))
        wxMessageBox("Error: CreateThread(StartNode) failed", "Ghostprotocol");
...
std::string HelpMessage()
{
    std::string strUsage = std::string(_("Options:\n")) +
        "  -conf=<file>     \t\t  " + _("Specify configuration file (default: ghostprotocol.conf)\n") +
        "  -pid=<file>      \t\t  " + _("Specify pid file (default: ghostprotocold.pid)\n") +
...
#ifdef USE_SSL
    strUsage += std::string() +
        _("\nSSL options: (see the ghostprotocol Wiki for SSL setup instructions)\n") +
